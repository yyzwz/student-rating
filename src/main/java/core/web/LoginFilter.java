...
public class LoginFilter implements Filter {
	...
	public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
		...
		if(url.startsWith("/sys/dsj")) {
			
		}
		else if (url.startsWith("/sys") && !url.contains("/sys/sysuser/login") && !url.contains("/sys/sysuser/findByName") && !url.contains("/sys/sysuser/home")&& !url.contains("/sys/sysuser/register")) {
			SysUser sysUser = (SysUser) request.getSession().getAttribute(Constant.SESSION_SYS_USER);
			if (null == sysUser) {
				response.sendRedirect(contextPath + "/loginExt.jsp");
				return;
			}
